...
describe('Search Facade Service', () => {
  ...
  it('should load the results data in store', inject(
    [Store],
    (store: Store) => {
  ...
  it('should be able to add item to cart', inject([Store], (store: Store) => {
    const spy = spyOn(store, 'dispatch').and.stub();
    facade.addToCart(bookMock);
    expect(spy).toHaveBeenCalled();
  }));

  it('should be able to remove item from cart', inject(
    [Store],
    (store: Store) => {
      const spy = spyOn(store, 'dispatch').and.stub();
  ...
  it('should be able to add user details to the collection', inject(
    [Store],
    (store: Store) => {
      const spy = spyOn(store, 'dispatch').and.stub();
      facade.attachBillingInfo(bookMock);
      expect(spy).toHaveBeenCalled();
    }
  ));

  it('should be able checkout all items in the cart', inject(
    [Store],
    (store: Store) => {
      const spy = spyOn(store, 'dispatch').and.stub();
      facade.checkOutCart();
  ...
  it('should be able to add item to the collections', inject(
    [Store],
    (store: Store) => {
      const spy = spyOn(store, 'dispatch').and.stub();
